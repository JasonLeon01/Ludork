...
| Template | Native source | Compiler needed for a normal Run | Use when |
|---|---|---|---|
...
C++ Source templates include a project-root `.clang-format` for native source formatting. They also require a first build to generate preview binaries and metadata. Standalone templates include the preview runtime in `Binaries` and its metadata in `Temp/UiPreview.json` and `Temp/UiPreview.registry.json`.
...
All four templates include the complete `Scripts/stub/**/*.d.lua` declaration tree for editors and Lua language tooling. These files are development assets. Packaging removes the entire `Scripts/stub` directory, and Lua compilation ignores `.d.lua`. No declaration file is shipped with a game.
