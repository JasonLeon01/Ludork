...
`PLATFORM` is the lower-case target name (`win32`, `darwin`, `ios`, `android`, `ohos`, or the CMake system name). `LUDORK_MOBILE` and `LUDORK_DESKTOP` expose the runtime host category. `SAVE_AS_LDC` is injected from the native `LUDORK_SAVE_AS_LDC` build setting and selects encrypted `.ldc` files for standard save slots. Do not infer a platform from paths or environment variables.
