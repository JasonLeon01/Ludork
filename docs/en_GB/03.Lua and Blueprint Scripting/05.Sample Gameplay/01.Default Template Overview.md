...
The Sample is both a playable reference and the source of the reusable templates. It demonstrates the expected project shape rather than a minimal empty application. Use it to learn ownership and data flow, and copy a pattern only after identifying its dependencies.
...
- `Scripts/Source/UI` contains Views generated from `Data/UI/Assets`. Handwritten foundations live in `Source.UIBase`, and business windows live in `Source.Windows`. See [Declarative UI Authoring Workflow](<../04.Declarative UI/01.Authoring Workflow.md>).
