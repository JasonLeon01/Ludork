...
The native ability system is exposed by `require("GlobalCore")`, including `AttributeSet`, `AbilitySystemComponent`, ability/result/event and Effect/spec types.

## AttributeSet

An `AttributeSet` subclass declares ordered `ATTRIBUTE_NAMES` and `{ type, default }` entries in `SCHEMA`; `ID` identifies the General Data member. Missing fields use schema defaults; an explicit stored nil branch overrides a default. Load stored General Data through `Source.Configs.GeneralDataTypes.Create`.
...
A constraint receives `(value, abilitySystem, resolvedValues)` after modifier aggregation, must return exactly one schema-valid finite number, and never writes Base. Passing nil removes it. Resolution follows `ATTRIBUTE_NAMES`, with HP last; `resolvedValues` contains fields already resolved in that pass.
