...
`Global.Gameplay` is the Sample's compact ability system. Require concrete modules directly and construct them with `ClassName.new(...)`.
...
`GameplayEventData.new(values?)` carries one activation or event.
