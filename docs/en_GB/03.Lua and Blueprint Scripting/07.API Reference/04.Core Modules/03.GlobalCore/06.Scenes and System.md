...
`getConfiguredScale()` returns the selected preference, including `0`; `getScale()` returns the current positive render and input-mapping scale. `setScale(value)` applies and saves, `applyScale(value)` applies without saving, and `saveScale(value)` saves without applying. Non-finite and negative values normalise to `1`; `0` selects borderless desktop fullscreen without changing the display mode. Standalone positive-scale windows are resizable and preserve the game aspect ratio with centred black bars. Manual resizing is session-only. Embedded displays remain host-sized and mobile retains its platform fullscreen behaviour, although the preference can still be saved.
