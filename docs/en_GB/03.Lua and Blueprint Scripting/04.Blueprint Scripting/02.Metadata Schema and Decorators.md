...
The capitalised `Dict[...]` and `Tuple[...]` tokens are metadata schema descriptions, not constructors for Standard's lower-case runtime `dict` and `tuple` containers. Metadata defaults remain JSON-convertible pure Lua tables; do not put a native `list`, `tuple` or `dict` value in `_METADATA`. Runtime containers must be converted explicitly before crossing a metadata, Blueprint, Core or JSON boundary.
...
A declared type is authoritative at runtime. Entry points do not evaluate strings or infer table, tuple or vector shapes to satisfy it. Convert historical serialised forms only in the data loader or schema-upgrade path, then pass the exact declared value, such as an `sf.Vector2i`, to the receiving API.
