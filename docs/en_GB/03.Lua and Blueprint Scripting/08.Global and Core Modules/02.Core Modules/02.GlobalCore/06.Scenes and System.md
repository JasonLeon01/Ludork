...
`getConfiguredScale()` returns the configured display preference, including `0`. `getScale()` returns the positive effective scale that rendering and input currently use after Maximum Render Scale is applied. The default configured preference is `1.0`. `setScale` applies the value and saves it. `applyScale` only applies the value, and `saveScale` only saves it. Non-finite or negative values normalise to `1`. On desktop, `0` selects borderless fullscreen without changing the display mode. At positive values, standalone desktop windows are resizable and preserve aspect ratio with centred black bars. Manual resizing of those windows is not persisted. On macOS, display scale is measured in Cocoa client points independently of the Retina backing scale.

`isDisplayScaleConfigurable()` is true for standalone desktop windows, including HarmonyOS 2in1, and false for embedded or ordinary mobile displays. In a HarmonyOS Mobile build for API 22 or newer, the result is true only on a tablet with free-window mode enabled. The ArkTS host listens for `freeWindowModeChange` and updates the native state. A positive value then requests a freeform Stage size of `round(gameSize * scale)`, while `0` requests a maximised or fullscreen window. Rendering uses the resulting surface. Manual platform resizing does not change the stored preference. HarmonyOS 2in1 follows the desktop scale protocol (default `1.0`).

`getMaximumWindowedScale(gameSize)` returns the largest scale that fits the available window dimensions. Desktop, including HarmonyOS 2in1, uses the decorated work area of the current window, or the primary display when no window exists. A configurable Mobile host supplies its own maximum and refreshes it when HarmonyOS free-window mode is enabled. The result is `nil` for embedded and non-configurable mobile hosts, for zero logical dimensions and for unknown available sizes.
