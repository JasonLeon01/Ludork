...
`Engine.JoystickButton` resolves logical controller buttons to the native button number for the current `PLATFORM`. The static getters `getA()`, `getB()`, `getX()`, `getY()`, `getLB()`, `getRB()`, `getView()`, `getMenu()`, `getLS()`, `getRS()`, `getXBox()` and `getShare()` return each available button as an `Engine.InputNamedValue`. Fixed button fields and a reverse lookup table are not available.
