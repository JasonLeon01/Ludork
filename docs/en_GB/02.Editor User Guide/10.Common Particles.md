...
Choose **Game → New Common Particle** or create a particle in the project file explorer. **Database → Common Particle Overview** (`F6`) lists the resources under `Data/Particles`. Select a resource to edit its tracks, properties and lifetime curves. Tracks support adding, copying, deleting and reordering; the preview can isolate the selected track. Track order also determines drawing order within an emitter.
