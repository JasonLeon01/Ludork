...
Each `drops` key must be a Blueprint class path whose generated Actor derives from `Source.Item`. The matching `sf.Vector2i` value is added to the defeated enemy's map position. The spawned Actor tag is derived from the Blueprint path and the absolute target coordinates. When an identical tag already exists, that drop is skipped and a warning is written instead of a suffixed duplicate.
