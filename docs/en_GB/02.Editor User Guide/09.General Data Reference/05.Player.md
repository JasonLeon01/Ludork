...
The Sample `Bravor` member defines `MAXHP = 2000` and `HP = 1000`. The runtime copies the selected member into `PlayerAttributeSet`. The `BP_Actor_Braver.json` Blueprint stores only the member `ID`.
