...
Applying a State creates an Infinite Gameplay Effect and grants the State's event abilities. `onWalk` listens for `Event.Movement.Step`. Trigger `onHookTriggered` explicitly with `Event.State.Trigger.<stateID>`, or call `NodeFunctions.Gameplay.SendEvent` inside a gameplay graph. Adding or stacking the State does not emit `onHookTriggered`. Activation returns `NoGraph` when the graph has no start node.
