...
Keep conventional first-level folders so selectors can filter correctly: `Tilesets`, `Autotiles`, `Characters`, `Animations`, `Icons`, `Fonts`, `Musics`, `Sounds`, `Voices`, `Shaders`, `Fogs`, `Transitions`, `System` and `Videos`. Custom groups are valid unless their names end in `.ldpak`. **Pack assets (.ldpak)** creates one archive per group and rejects ordinary files directly under `Assets`.
