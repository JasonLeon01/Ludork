...
`packaging.version` stores the base release version and `packaging.dev` selects internal test packages. Missing values default to `"1.0.0"` and `false`. Use Pack Options to save them, or override them for one command-line invocation; see [Release version and internal packages](<08.Run Debug and Package.md#release-version-and-internal-packages>).
