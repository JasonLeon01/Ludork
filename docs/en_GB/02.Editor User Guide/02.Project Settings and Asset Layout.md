...
Anti-aliasing defaults to SFML's own context default and applies on the next display initialisation. Maximum Render Scale defaults to `2.0`; Unlimited (`0`) removes the cap. Scale appears only when the runtime host supports display scaling. Desktop Scale defaults to `1.0`. In editor mode the project copy is used; packaged games keep mutable settings in their platform user-data location.
