...
An ordinary map is `Data/Maps/<Name>.json`. A world is `Data/Maps/<WorldName>/` with exactly one `_world.json` and direct child-map `.json` files; no deeper nesting is allowed. The manifest is the world's selector/save identity and stays hidden in the Map List.
...
`layerOrder` is derived from placed children and rejects contradictory cycles. The manifest owns only world size, composition and global fog; audio, filters and ambient light remain child data.
