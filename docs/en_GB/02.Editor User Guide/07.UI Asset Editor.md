...
Preview instantiates the asset tree, not its Lua Controller, so top-level assets contain every visible pane and representative placeholders. `previewText` and `editor` data are design-only; Controllers replace placeholders before display.
