...
If you are extending Ludork itself, the repository is organized roughly as:

- `main.py` / `LoadEditor.py` — editor entry points.
- `Global/` — global editor state, startup and main window glue.
- `Widgets/` — editor UI panels and dialogs (with shared pieces in `Widgets/Utils/`).
- `NodeGraph/` — editor-side Node Graph integration and node definitions.
- `Sample/Engine/` — the reference runtime engine implementation:
  - `EngineSystem.py` / `EngineSceneBase.py` — runtime bootstrap and scene base.
  - `Gameplay/`, `UI/`, `Manager/`, `NodeGraph/`, `Utils/`, `Filters/` — core runtime subsystems.
- `C_Extensions/` — pybind11-based native acceleration modules.
- `EditorExtensions/` — build output location for editor-side native modules (often populated by build scripts rather than committed).
