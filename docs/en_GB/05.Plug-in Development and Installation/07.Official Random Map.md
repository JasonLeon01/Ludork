...
`OfficialRandomMapPlugin` registers exactly one `PluginMapContextMenuCommand`. It does not register a normal menu command, a text hint provider, a before-Run hook or a before-Pack hook. The command is displayed only when an actual map item is right-clicked.
