...
The five registrar methods cover ordinary menu commands, map-item context-menu commands, text hint providers, before-Run hooks and before-Pack hooks. There are no public registrations for docked panels, toolbars, arbitrary host controls, data types or general editor events. Runtime extension belongs in [Native C++ Development](<../04.Native C++ Development/01.Build and Module Layout.md>), not in editor plug-ins.
...
Official Blueprint AI, Official Locale Tools and Official Random Map are distributed together with the editor. The Windows distribution installs all three below the installation root's `Plugins` folder and includes a `plugins.json` generated from their manifests. They are already registered and enabled, so their release installation does not show the third-party trust confirmation.
