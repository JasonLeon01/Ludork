...
地图列表包含普通地图和大地图。展开大地图显示直接子地图，`_world.json` 保持隐藏；选择大地图打开拼合面板。右键菜单可创建各类资产。复制子地图会分配新的 Actor MapTag，但不自动摆放副本。
...
校验覆盖大地图清单、目录结构、placement、图层顺序和全局 Actor MapTag 唯一性。会使 placement 失效的缩放直接被拒绝，不会自动移动或裁切；删除子地图会移除 placement。
...
`startMap` 文件字段和 Blueprint Transfer 选择器使用同一棵地图树。选择大地图时保存 `<大地图名称>/_world.json`，position 使用大地图格坐标；选择已摆放子地图时保存 `<大地图名称>/<子地图名称>.json`，position 使用该子地图的局部坐标，并由运行时换算为大地图坐标。未摆放子地图仍会显示在树中，但不能选作运行时目标。
