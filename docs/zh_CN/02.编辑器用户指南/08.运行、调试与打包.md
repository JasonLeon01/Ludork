...
- 目标满足平台最低要求：Windows 10 或更高版本（x64）、macOS 13.3 或更高版本（Apple Silicon）、iOS 15.0 或更高版本（arm64）、HarmonyOS 6.0.1 / API 21 或更高版本（arm64-v8a），或 Android 7.0 / API 24 或更高版本（arm64-v8a）。
...
HarmonyOS Mobile 打包需要 C++ Source 项目、Apple Silicon macOS，以及包含 OpenHarmony native SDK 的 DevEco Studio。其最低目标为 HarmonyOS 6.0.1 / API 21，生成 arm64-v8a 的未签名 HAP 到 `dist`；暂不可用的 2in1 目标不会显示。C++ Source + FFmpeg 的 HarmonyOS 构建会静态链接 FFmpeg 并携带工程运行时法律材料；下游发行必须满足 `Licenses/FFmpeg/README.md` 所述的适用 LGPL 声明与重链接材料要求。
...
Run 或 Pack 停止时，先阅读 Console 或打包日志中的第一个可执行错误。依次检查插件 hook、`Main.proj`、入口脚本、JSON 语法、Blueprint 校验、生成的原生模块和缺失资源。Android 签名失败时，还要确认 keystore 路径为绝对路径、文件仍可读、alias 存在且两个密码与所选 key 匹配；凭据不会跨 Pack 操作保留，因此需要重新填写。

## 预期结果

编辑器运行与打包应用启动同一入口脚本，使用预期资产与生成 Lua 数据。发行者还必须单独确认最终包或随附发行材料提供了所选 capability 要求的全部声明。
...
macOS 签名与公证由发行者负责。设备导出需要已正确准备的实体 iPhone。Android 输出默认未签名，只有勾选**签名 APK**时才会签名；两种 Android 流程都不执行设备或模拟器测试。打包不能修复无效项目引用，也不能替代目标平台测试。
