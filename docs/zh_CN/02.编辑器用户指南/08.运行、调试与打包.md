...
HarmonyOS 打包需要 C++ Source 项目、Apple Silicon macOS，以及包含 OpenHarmony native SDK 的 DevEco Studio。所有 HarmonyOS 变体均为 HarmonyOS 6.0.2 / API 22。选择 **Mobile** 使用 OpenGL ES 手机/平板宿主，或选择 **2in1** 后再选 OpenGL（默认）或 OpenGL ES。

不导出到设备时，打包器会在 `dist` 中生成一个 arm64-v8a 未签名 HAP：`<游戏名>-harmony-mobile-unsigned.hap`、`<游戏名>-harmony-2in1-opengl-unsigned.hap` 或 `<游戏名>-harmony-2in1-opengl-es-unsigned.hap`。勾选**导出到匹配的 HarmonyOS 设备**后，会改为生成对应的 `-signed.hap`，将其安装到设备并启动游戏。Mobile 导出接受报告为 `default`、`phone` 或 `tablet` 的已连接设备；2in1 导出只接受 `2in1`。必须恰好有一台已连接设备匹配所选形态；另一形态的设备可以保持连接。2in1 OpenGL 包要求设备上有 `libGLv4.so`。部分模拟器没有该库；应用会报告模块缺失且不回退到 OpenGL ES，此时应改用 OpenGL ES 包。
...
Android 资源只最终化一次，以带 SHA-256 manifest 的 APK assets 保存，并在原生启动前复制到版本化 runtime 目录。打包器会包含已有的根许可证/声明文件，但不强制其存在。生成工程从根目录 `Licenses` 获取适用运行时正文，不携带编辑器声明。静态链接 FFmpeg 不会免除 `Licenses/FFmpeg/README.md` 中的 LGPL 声明与重链接义务。
