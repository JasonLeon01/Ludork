...
HarmonyOS Mobile 打包需要 C++ Source 项目、Apple Silicon macOS，以及包含 OpenHarmony native SDK 的 DevEco Studio。其最低目标为 HarmonyOS 6.0.1 / API 21，生成 arm64-v8a 的未签名 HAP 到 `dist`；暂不可用的 2in1 目标不会显示。C++ Source + FFmpeg 的 HarmonyOS 构建会静态链接 FFmpeg 并携带工程运行时法律材料；下游发行必须满足 `Licenses/FFmpeg/README.md` 所述的适用 LGPL 声明与重链接材料要求。`Licenses` 与根许可证/声明文件不是 HAP 格式前置条件：存在时打包器会包含它们，缺失时不会拒绝项目。
