...
打开 **Database → Game Variables**，声明由 Blueprint 与当前游戏实例共享的可变值。左侧列出已声明的变量名；选中一项后，可在右侧编辑类型、初始值和可选备注。备注只供编辑器查看，不影响运行时。变量名是运行时标识符，因此 graph、地图或脚本开始引用后应保持稳定。
...
General Data member 应用到 Info 对象或通过 `BPBase.ApplyGeneralData` 应用时，每个 string 都会保持存储原值。string 字段不会自动本地化，因为同一字段类型还用于 class、animation、装备槽位等语义标识符。类似 `{ITEM_NAME}` 的花括号显示值只应在展示边界传给 `LOC`，长生命周期 UI 响应语言变化时应重新解析。Catalogue 格式与 runtime formatter 见“插件系统与本地化”。
