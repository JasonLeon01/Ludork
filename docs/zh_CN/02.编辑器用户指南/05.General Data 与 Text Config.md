...
每个字段定义都可以保存可选的英文 `comment`。新增字段时通过 **Comment** 填写；空注释不会写入。Form 字段名和 Table 表头会在悬停时显示该注释。这个 schema 注释与恰好名为 `desc` 的 member 业务字段值不是同一内容，并且只是项目中的普通文本，不是 locale key。
...
- 分别创建每种 SFML 字段以及 typed list/dict，重新打开文件后检查 JSON 值与对应控件。
