...
| `billboardComp` | `{ "Engine", "BillboardComponent" }` | — | component；[头顶显示](#billboardcomponent) |
...
| `ExecuteGraph` | `function` | graph: Engine.Graph; eventName: string; keywordArguments: any = nil; localGraph: any = nil; onComplete: any = nil | return: bool | 运行时 API；metadata = false |
...
`Engine.BillboardComponent` 继承 `Component`，配置在 `Actor.billboardComp` 字段。

| 属性 | 类型 | 默认值 | 含义 |
|---|---|---|---|
| `items` | `Engine.BillboardItem[]` | `[]` | 图片、文字项，按顺序从上到下排列。 |
| `showRange` | `float` | `128` | 玩家与 Actor 世界位置的最大距离，单位为地图逻辑像素，包含边界。 |

### BillboardItem

| 属性 | 类型 | 默认值 | 含义 |
|---|---|---|---|
| `kind` | `string` | `"text"` | `text` 或 `image`。 |
| `text` | `string` | `""` | 文字内容。 |
| `fontSize` | `uint` | `12` | 文字字号。 |
| `color` | `sf.Color` | `[255,255,255,255]` | 文字 RGBA 颜色。 |
| `image` | `string` | `""` | 图片资源路径。 |

文字使用系统默认字体，图片使用原始像素尺寸。各项共享 Actor 头顶的水平中心线，多行文字逐行居中，项间无额外间距。稳定显示时，最下面一项贴住 Actor 图像包围盒的顶部中心。整体保持直立，不继承 Actor 的旋转与翻转。

### 可见性与动画

显示要求玩家进入 `showRange`，且 Actor 包围盒与 Camera 视野相交；同时遵守 Actor、祖先、地图区域活动状态和图层可见性，不受 `tickable` 影响。Billboard 在 Actor 及其附属 emitter 之后绘制，可被后续 Actor 和图层遮挡，并接受地图窗口裁剪。

淡入和淡出均上升，持续 `0.35` 秒，由四条系统曲线控制。默认关键帧采用线性插值和常量外推：

| 曲线资源 | 0 → 0.35 秒的值 |
|---|---|
| `Global/BillboardYIn` | `16 → 0` 逻辑像素 |
| `Global/BillboardAlphaIn` | `0 → 255` |
| `Global/BillboardYOut` | `0 → -16` 逻辑像素 |
| `Global/BillboardAlphaOut` | `255 → 0` |

动画透明度与每项原有透明度相乘。转换途中反向时，从当前位置和透明度平滑过渡，重新计时 `0.35` 秒。离开视野、隐藏或休眠会重置显示，返回时重新淡入。
