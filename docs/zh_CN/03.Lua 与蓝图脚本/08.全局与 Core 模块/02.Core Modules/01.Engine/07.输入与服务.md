...
`Engine.Input.captureInput()` 返回捕获句柄，阻断普通输入和 UI 交互，更新、渲染与 latent 调度继续运行。`Engine.Input.isInputCaptured()` 包含所有捕获及等待已按住控件释放的状态。

确认默认关闭，通过 `handle:setConfirmEnabled(true)` 开启，再用 `handle:consumeConfirm()` 读取。只有新的 Enter/Space、鼠标左键、手柄逻辑 A/Cross 按下及单指 tap 可以确认。tap 必须在开启后于指针视口内开始，并在同一活动句柄上结束；拖动和多指会取消它。切换确认开关会清除待处理确认和进行中的 tap。

取消或销毁时调用 `handle:release()`；该方法可重复调用，句柄销毁也会释放捕获。只有最新的活动句柄接收确认，WebView 优先。释放一个句柄不影响其他捕获；关闭当帧及已按住控件的释放过程仍被拦截，防止输入穿透。
