...
`getConfiguredScale()` 返回配置的显示偏好，取值可以是 `0`。`getScale()` 返回应用最大渲染缩放之后，渲染与输入当前使用的正数有效缩放。配置偏好的默认值为 `1.0`。`setScale` 应用该值并保存。`applyScale` 只应用，`saveScale` 只保存。非有限值或负数一律归一为 `1`。桌面端用 `0` 选择无边框全屏，且不改变显示模式。取正数值时，桌面独立窗口可以调整大小，并保持宽高比、以黑边居中。这类窗口的手动调整不会持久化。macOS 上显示缩放以 Cocoa 客户区 point 计量，与 Retina backing scale 无关。

`isDisplayScaleConfigurable()` 对独立桌面窗口（包括 HarmonyOS 2in1）返回 true，对嵌入式显示和普通移动端显示返回 false。在 API 22 或更高版本的 HarmonyOS Mobile 构建中，只有启用自由多窗模式的平板返回 true。ArkTS 宿主监听 `freeWindowModeChange`，并据此更新原生状态。此时正数值请求 `round(gameSize * scale)` 的自由窗口 Stage 尺寸，`0` 则请求最大化或全屏窗口。渲染使用最终获得的 surface。手动调整平台窗口不会改变已保存的偏好。HarmonyOS 2in1 遵循桌面缩放协议（默认 `1.0`）。

`getMaximumWindowedScale(gameSize)` 返回可用窗口尺寸所能容纳的最大缩放。桌面端（包括 HarmonyOS 2in1）使用当前窗口的装饰工作区；没有窗口时改用主显示器。可配置的 Mobile 宿主自行提供上限，并在 HarmonyOS 启用自由多窗模式时刷新。嵌入式宿主、不可配置的移动端宿主、逻辑尺寸为零以及可用尺寸未知时，结果均为 `nil`。
