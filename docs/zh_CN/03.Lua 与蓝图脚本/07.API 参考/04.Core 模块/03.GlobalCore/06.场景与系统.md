...
`requestTransition` 接受空的可选值或完整 `/Game/Assets/Transitions/...` 逻辑路径，不会自动补类别前缀。
...
`getMaximumRenderScale()` 返回有效缩放上限；`0` 表示不限，默认值为 `1`，无效值归一为 `2`。`setMaximumRenderScale` 在帧间应用并保存且不改变窗口尺寸；`saveMaximumRenderScale` 只保存。

### 光照渲染倍率语义

`getLightingRenderScale()` 返回 `0.5`、`0.75` 或 `1`；默认值为 `0.75`，无效值归一为 `1`。`setLightingRenderScale` 应用并保存、重建直接光照目标并使静态缓存失效；`saveLightingRenderScale` 只保存。Surface Mask、Transmission 与 Occupancy 保持全分辨率。

### 抗锯齿语义

抗锯齿默认值为 `2`，无效值使用 SFML 默认值。`setAntiAliasingLevel` 只保存、不重建当前上下文，因此在下次初始化显示时生效；SFML 可能协商为其它受支持等级。

### 帧率语义

`getFrameRate()` 返回配置的目标帧率，默认 `60`；`0` 表示不限，关闭主动帧率限制。垂直同步保持独立，仍可能限制画面呈现速率。`setFrameRate` 应用并保存，`saveFrameRate` 只保存。不限时固定更新步长为 `1/60` 秒；正数目标帧率的固定更新步长为 `1/framerate` 秒。
