...
`getConfiguredScale()` 返回配置偏好（可为 `0`）；`getScale()` 返回当前用于渲染和输入的正数缩放，默认偏好为 `1.0`。`setScale` 应用并保存，`applyScale` 只应用，`saveScale` 只保存；非有限值或负数归一为 `1`，`0` 表示不切换显示模式的桌面无边框全屏。正数缩放下，独立窗口可调整大小、保持宽高比并以黑边居中，手动调整不持久化。macOS 以 Cocoa 客户区 point 计量缩放，不受 Retina backing scale 叠加。Embedded 保持宿主尺寸；移动端保持平台全屏，但仍可保存偏好。

`getMaximumWindowedScale(gameSize)` 返回逻辑尺寸为 `gameSize` 的普通带装饰窗口在当前窗口所在显示器工作区内可使用的最大缩放；没有窗口时查询主显示器。任一尺寸为零、处于 embedded 或 mobile 模式，或无法取得工作区时返回 `nil`。
