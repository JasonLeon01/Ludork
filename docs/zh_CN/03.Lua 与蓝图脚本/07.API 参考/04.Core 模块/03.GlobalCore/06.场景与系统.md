...
`isDisplayScaleConfigurable()` 对桌面独立窗口返回 true，对 embedded 与普通移动端显示返回 false。在 HarmonyOS API 22 或更高版本上，仅启用自由多窗模式的平板返回 true；ArkTS 宿主监听 `freeWindowModeChange` 并更新原生状态。正数值请求 `round(gameSize * scale)` 的自由 Stage 尺寸，`0` 请求最大化/全屏。渲染使用最终取得的 surface，手动调整平台窗口不会改变已保存偏好。

`getMaximumWindowedScale(gameSize)` 返回可用窗口能容纳的最大缩放。桌面端使用当前窗口所在显示器的带装饰工作区，无窗口时使用主显示器；可配置移动端由宿主报告上限，HarmonyOS 在启用自由多窗模式时刷新。Embedded、不可配置移动端、零逻辑尺寸或未知可用尺寸均返回 `nil`。
