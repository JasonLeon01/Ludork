...
Context accessor 和 context-bound mutation 节点要求 General Data graph 的 `GameplayEventData` parent；用于普通 Actor graph 会报错。`RemovePlayerState` 与 `ReducePlayerState` 作用于该 context 的 target，State 不存在时仍保留单一执行输出。

动态 attribute pin 使用 `any`，但 Ability System 仍要求符合所选 schema 类型的有限 number。`SetNumericAttributeBase` 写入 Base；`ApplyAttributeDelta` 应用 Instant Add modifier。
