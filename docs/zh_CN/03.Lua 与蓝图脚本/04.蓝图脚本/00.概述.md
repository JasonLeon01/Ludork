...
通过 **游戏 → 新建蓝图**创建 Blueprint，选择有效父类，添加受支持的事件入口，连接执行与数据 pin，然后校验并保存。多个图需要相同操作时，按 F7 打开 Common Functions。Script Mixin Blueprint 遵循独立的 [Script Mixin](<../03.脚本 Mixin/00.概述.md>)契约，不执行其图。
