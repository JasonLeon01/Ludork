...
`ResourceCleanupProgress` 包含阶段、已完成数量、总数和当前路径。报告存在扫描问题时不能回收；项目输入变化会使报告失效。系统回收站或垃圾篓不可用时，宿主不会退回永久删除。扫描边界与检查流程见 [Official Resource Cleanup](<10.Official Resource Cleanup.md>)。
