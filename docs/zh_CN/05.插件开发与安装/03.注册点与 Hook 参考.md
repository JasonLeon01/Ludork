...
注册只能在 `IEditorPlugin.Register` 中进行。API 共提供五个注册方法。
...
`RegisterBeforeRunHook` 与 `RegisterBeforePackHook` 接受 `IProjectOperationHook`。hook 在目标操作之前依次执行。context 提供项目路径、语言、操作类型、输出写入器、文本提示失效与取消信号。

结果失败或抛出异常会终止后续 hook，并阻止 Run 或 Pack。hook 必须响应取消请求，并通过 `IPluginOutput` 写出进度。
...
| `Packaging` | 仅 before-Pack hook 中非 null；before-Run 中为 null |
| `Output` | 只追加的操作输出 |
| `TextHints` | 请求刷新文本提示 |
...
Official Locale Tools 在 Pack 时调用 `ExcludeFile("Data/Locale/Locale.xlsx")`，使创作用的本地化工作簿不被分发。Run 期间 `Packaging` 为 null，因此它不会调用该方法。
