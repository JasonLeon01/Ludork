...
class MoveRouteMapView(QtWidgets.QWidget):
    ...
    def _drawTileLayer(
        self, painter: QtGui.QPainter, layer: Dict[str, Any], width: int, height: int, sourceTileSize: int
    ) -> None:
        ...
        for y, row in enumerate(autoTiles):
            ...
            for x, key in enumerate(row):
                ...
                if tile is None or tile.isNull():
                    continue
                painter.drawImage(
                    QtCore.QRect(x * self._tileSize, y * self._tileSize, self._tileSize, self._tileSize), tile
                )
