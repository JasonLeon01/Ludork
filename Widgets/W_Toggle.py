...
class ModeToggle(QtWidgets.QWidget):
    ...
    def _drawEditorIcon(self, p: QtGui.QPainter, rect: QtCore.QRect):
        ...
        p.drawRoundedRect(r, int(4 * self._scale), int(4 * self._scale))
        fold = int(6 * self._scale)
        ...
        pb.setWidth(max(1, int(2 * self._scale)))
        p.setPen(pb)
        x1 = x0 + int(s * 0.25)
        y1 = y0 + int(s * 0.70)
        x2 = x0 + int(s * 0.85)
        ...
        tip = QtGui.QPolygon([
            QtCore.QPoint(x2, y2),
            QtCore.QPoint(x2 - int(6 * self._scale), y2 + int(2 * self._scale)),
            QtCore.QPoint(x2 - int(2 * self._scale), y2 + int(6 * self._scale)),
        ])
        p.drawPolygon(tip)
...
class EditModeToggle(QtWidgets.QWidget):
    ...
    def _drawTileIcon(self, p: QtGui.QPainter, rect: QtCore.QRect):
        ...
        for j in range(rows):
            for i in range(cols):
                r = QtCore.QRect(x0 + i * cell + int(2 * self._scale), y0 + j * cell + int(2 * self._scale), cell - int(4 * self._scale), cell - int(4 * self._scale))
                p.drawRoundedRect(r, int(3 * self._scale), int(3 * self._scale))
    ...
    def _drawActorIcon(self, p: QtGui.QPainter, rect: QtCore.QRect):
        ...
        r_head = max(3, int(6 * self._scale))
        p.drawEllipse(QtCore.QPoint(cx, cy - int(s * 0.22)), r_head, r_head)
        p.setBrush(QtCore.Qt.NoBrush)
        shoulders = QtGui.QPainterPath()
        bw = int(18 * self._scale)
        bh = int(10 * self._scale)
        x0 = cx - bw // 2
        y0 = cy + int(s * 0.02)
        shoulders.addRoundedRect(QtCore.QRectF(x0, y0, bw, bh), int(4 * self._scale), int(4 * self._scale))
