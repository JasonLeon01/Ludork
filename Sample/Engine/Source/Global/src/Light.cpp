...
Light::Light(const sf::Vector2f& position, const sf::Color& colour,
...
LightData Light::asDict() const {
    return {
        ...
        {"color", std::vector<int>{static_cast<int>(colour.r),
...
Light Light::fromDict(const LightData& data) {
    ...
    if (colorIt != data.end()) {
        ...
        light.colour =
            sf::Color(clampColor(colorValues[0]), clampColor(colorValues[1]),
                      clampColor(colorValues[2]), clampColor(colorValues[3]));
    }
