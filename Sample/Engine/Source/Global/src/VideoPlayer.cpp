...
VideoPlayer::VideoPlayer(std::string path, bool mute, bool skipable)
    : path_(std::move(path)),
      mute_(mute),
      skipable_(skipable),
      decoder_(path_),
...
void VideoPlayer::update(sf::RenderWindow& window,
...
std::optional<int> VideoPlayer::getFrame(sf::RenderWindow& window) {
...
void VideoPlayer::updateSpriteLayout(const sf::RenderWindow& window) {
...
void runVideoPlayback(const std::string& path, bool mute, bool skipable) {
    VideoPlayer(path, mute, skipable).play();
}
#else
