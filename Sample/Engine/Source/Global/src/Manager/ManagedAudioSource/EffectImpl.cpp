#include "ProcessorGeneration.hpp"
...
bool ProcessorGeneration::tryAcquireActivity() noexcept {
...
void ProcessorGeneration::releaseActivity() noexcept {
    const std::uint32_t previous = gate.fetch_sub(1, std::memory_order_acq_rel);
    if ((previous & ActivityMask) == 1) {
        gate.notify_all();
    }
}

void ProcessorGeneration::cancel() noexcept {
    gate.fetch_or(CancelledBit, std::memory_order_acq_rel);
    control->cancel();
}

void ProcessorGeneration::waitUntilInactive() const noexcept {
    std::uint32_t state = gate.load(std::memory_order_acquire);
...
static_assert(std::atomic<EffectImpl::ManagedInputState>::is_always_lock_free);
...
EffectImpl::EffectImpl(const sf::SoundSource& source) noexcept
    : source_(&source) {}

sf::SoundSource::EffectProcessor EffectImpl::makeTrampoline() {
    return [this](const float* inputFrames, unsigned int& inputFrameCount,
        ...
        struct CallbackScope {
            ...
            ~CallbackScope() {
                if (impl->callbacksInFlight_.fetch_sub(
                        1, std::memory_order_acq_rel) == 1) {
                    impl->callbacksInFlight_.notify_all();
                }
                leaveCallback();
            }
        } callbackScope{this};
...
void EffectImpl::begin(const std::shared_ptr<EffectStateToken>& control) {
...
void EffectImpl::replace(sf::SoundSource::EffectProcessor effectProcessor) {
...
void EffectImpl::abort() noexcept {
    std::shared_ptr<ProcessorGeneration> cancelled;
    {
        const std::lock_guard<std::mutex> lock(attachmentMutex_);
...
void EffectImpl::cancel() noexcept {
    std::shared_ptr<ProcessorGeneration> cancelled;
    {
        const std::lock_guard<std::mutex> lock(attachmentMutex_);
        stopping_ = true;
...
void EffectImpl::clear() noexcept {
    std::shared_ptr<ProcessorGeneration> cancelled;
    {
        const std::lock_guard<std::mutex> lock(attachmentMutex_);
        cancelled = retireCurrentLocked({});
...
void EffectImpl::preparePlaying() noexcept {
    {
        const std::lock_guard<std::mutex> lock(attachmentMutex_);
...
void EffectImpl::markPlaying() noexcept {
    const std::lock_guard<std::mutex> lock(attachmentMutex_);
    if (processorOwner_ != nullptr) {
...
void EffectImpl::notifyNaturalInputEnded() noexcept {
    const std::lock_guard<std::mutex> lock(attachmentMutex_);
...
bool EffectImpl::isNaturalInputDrained() const noexcept {
    const std::lock_guard<std::mutex> lock(attachmentMutex_);
...
RetiredProcessorGenerations EffectImpl::takeAllRetired() noexcept {
    const std::lock_guard<std::mutex> lock(attachmentMutex_);
    return RetiredProcessorGenerations(std::move(retired_));
}

void EffectImpl::waitForCallbacks() const noexcept {
...
bool EffectImpl::wasExplicitlyStopped() const noexcept {
    return explicitlyStopped_.load(std::memory_order_acquire);
}

void EffectImpl::completeNaturalInput(
...
void EffectImpl::waitUntilInactive(
...
std::shared_ptr<ProcessorGeneration> EffectImpl::retireCurrentLocked(
