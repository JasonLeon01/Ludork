...
struct LUDORK_ENGINE_API Material {
    BIND_PROPERTY()
    float lightBlock = 0.0f;  ///< Amount of light blocked (0.0 =
    ...
    BIND_PROPERTY()
    float opacity =
        1.0f;  ///< Visual opacity (0.0 = invisible, 1.0 = fully visible)

    BIND_PROPERTY()
    float speedRate =
        1.0f;  ///< Movement speed multiplier for actors on this surface

    BIND_PROPERTY()
    bool ignoreLighting = false;  ///< Whether the surface is rendered without
                                  ///< ambient or direct lighting

    ////////////////////////////////////////////////////////////
    /// \brief Construct a material object
    ///
    /// - \param inLightBlock Amount of light blocked (0.0 = transparent, 1.0 =
    /// fully opaque)
    ...
    /// - \param inSpeedRate Movement speed multiplier for actors on this
    /// surface
    /// - \param inIgnoreLighting Whether to render without ambient or direct
    /// lighting
    ////////////////////////////////////////////////////////////
    BIND_INIT()
