...
private:
    RuntimeData parseValue() {
        ...
        switch (text_[position_]) {
            ...
            case 't':
                consumeLiteral("true");
                return RuntimeData(true);
            case 'f':
                consumeLiteral("false");
                return RuntimeData(false);
            case '"':
                return RuntimeData(parseString());
            case '[':
                return RuntimeData(parseArray());
            case '{':
                return RuntimeData(parseObject());
            default:
                return parseNumber();
        }
    }

    RuntimeData::Array parseArray() {
        ++position_;
        skipWhitespace();
    ...
    RuntimeData parseNumber() {
        ...
        return RuntimeData(static_cast<std::int64_t>(std::stoll(token)));
    }
...
void writeValue(std::ostream& output, const RuntimeData& value) {
    if (value.isNil()) {
        output << "null";
    ...
    } else if (const RuntimeData::Array* array =
                   value.getIf<RuntimeData::Array>()) {
        output.put('[');
        ...
        output.put(']');
    } else if (const RuntimeData::Map* map = value.getIf<RuntimeData::Map>()) {
        output.put('{');
        bool first = true;
...
RuntimeData getJSONData(const std::string& filePath) {
    return getJSONData(ludork::standard::pathFromUtf8(filePath));
}

RuntimeData getJSONData(const std::filesystem::path& filePath) {
    return parseJSONText(getJSONText(filePath));
}

RuntimeData parseJSONText(const std::string& text) {
    return JsonParser(text).parse();
}

std::string stringifyJSON(const RuntimeData& value) {
...
void writeJSON(const std::string& filePath, const RuntimeData& value) {
