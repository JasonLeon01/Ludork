...
RuntimeValue data(const RuntimeValue& value) {
    RuntimeScope scope;
...
RuntimeValue table(WeakMode mode) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    const char* weak = weakMode(mode);
    return read(sol::make_object(
        lua, weak == nullptr ? lua.create_table()
                             : detail::createWeakTable(lua, weak)));
}

RuntimeValue globals() {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    return read(sol::make_object(lua, lua.globals()));
}

RuntimeValue registry() {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    return read(sol::make_object(lua, lua.registry()));
}

RuntimeValue registryTable(const std::string& key, WeakMode mode) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    return read(sol::make_object(
...
RuntimeValue get(const RuntimeValue& target, const RuntimeValue& key) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    return read(
...
RuntimeValue rawGet(const RuntimeValue& target, const RuntimeValue& key) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    return read(
...
void set(const RuntimeValue& target, const RuntimeValue& key,
...
void rawSet(const RuntimeValue& target, const RuntimeValue& key,
            const RuntimeValue& value) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    detail::runtimeAssign(lua, write(lua, target), write(lua, key),
...
Entries entries(const RuntimeValue& target) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
...
RuntimeValue::Array keys(const RuntimeValue& target, RuntimeLookupMode mode) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
...
std::size_t length(const RuntimeValue& target) {
    RuntimeScope scope;
...
bool hasMetatable(const RuntimeValue& value) {
    RuntimeScope scope;
...
RuntimeValue metatable(const RuntimeValue& value) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    return read(
...
void setMetatable(const RuntimeValue& value,
                  const RuntimeValue& valueMetatable) {
    RuntimeScope scope;
    lua_State* state = scope.state();
...
bool isCallable(const RuntimeValue& value) {
    ...
    return (type == "table" || type == "userdata") &&
           isFunction(rawGet(metatable(value), "__call"));
}

RuntimeValue::Array invoke(const RuntimeValue& callable,
...
RuntimeValue callback(Callback function) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    ...
    return read(sol::make_object(lua, sol::as_function(std::move(wrapper))));
}
...
std::vector<std::string> functionParameterNames(const RuntimeValue& method) {
    ...
    const RuntimeValue rawGetInfo = rawGet(debug, "getinfo");
    const RuntimeValue rawGetLocal = rawGet(debug, "getlocal");
    if (!isFunction(rawGetInfo) || !isFunction(rawGetLocal)) {
        return names;
    }
    RuntimeValue getInfo = rawGetInfo;
    RuntimeValue::Array infoResult =
        invoke(getInfo, {makeValue(method), makeValue("u")});
    const RuntimeValue rawInfo = first(infoResult);
    if (!isTable(rawInfo)) {
        return names;
    }
    const RuntimeValue rawCount = rawGet(rawInfo, "nparams");
    ...
    for (std::size_t index = 1; index <= count; ++index) {
        RuntimeValue::Array localResult =
            invoke(getLocal, {makeValue(method), makeValue(index)});
...
std::optional<FunctionSource> functionSource(const RuntimeValue& callable) {
    ...
    const RuntimeValue getInfo = rawGet(debug, "getinfo");
    if (!isFunction(getInfo)) {
        return std::nullopt;
    }
    const RuntimeValue info =
        first(invoke(getInfo, {callable, makeValue("S")}));
    if (!isTable(info)) {
        return std::nullopt;
    }
    const RuntimeValue what = rawGet(info, "what");
    const RuntimeValue source = rawGet(info, "source");
    const RuntimeValue firstLine = rawGet(info, "linedefined");
    const RuntimeValue lastLine = rawGet(info, "lastlinedefined");
...
bool moduleExists(const std::string& module) {
    ...
    for (const char* field : {"loaded", "preload"}) {
        const RuntimeValue modules = rawGet(package, field);
        if (isTable(modules) && !rawGet(modules, module).isNil()) {
            return true;
        }
    }
    const RuntimeValue search = rawGet(package, "searchpath");
    if (!isFunction(search)) {
        return false;
    }
    for (const char* field : {"path", "cpath"}) {
        const RuntimeValue path = rawGet(package, field);
        if (!is<std::string>(path)) {
            continue;
        }
        try {
            if (is<std::string>(
                    first(invoke(search, {makeValue(module), path})))) {
                return true;
            }
...
RuntimeValue finalizeClass(const RuntimeValue& definition,
...
RuntimeValue::Array classMro(const RuntimeValue& value) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    const sol::table mro =
...
RuntimeValue typeMetadata(const RuntimeValue& value) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    return read(
...
bool isClass(const RuntimeValue& value) {
    return isTable(value) && is<bool>(rawGet(value, "__ludorkClass")) &&
           as<bool>(rawGet(value, "__ludorkClass"));
}

bool isNativeType(const RuntimeValue& value) {
    return isTable(value) && !isClass(value) &&
           isTable(rawGet(metatable(value), "__type"));
}
...
bool hasOwnField(const RuntimeValue& target, const RuntimeValue& key) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
...
void setNativeDefaultResolver(Callback resolver) {
    RuntimeScope scope;
    sol::state_view lua(scope.state());
    const RuntimeValue function = callback(std::move(resolver));
