...
sol::object clonePlainDataImpl(sol::state_view lua,
...
sol::object constructRuntimeClass(sol::state_view lua,
    ...
    try {
        ...
        ensureRuntimeLuaStack(
            state, static_cast<std::size_t>(argumentCount) + 1,
...
int runtimeClassResolverImpl(lua_State* state) {
    ...
    const std::string operation =
        sol::stack::get<std::string>(state, 1);
    const RuntimeArguments arguments(state, 2, std::max(lua_gettop(state) - 1, 0));
    ...
    if (operation == "reflect.isInstance") {
        const bool result = second.is<sol::table>() &&
                            isInstance(sol::this_state(state), first,
                                       second.as<sol::table>());
        return runtimeResolverResult(lua, {sol::make_object(lua, result)});
    }
...
int invoke(lua_State* state, const sol::object& callable,
           int argumentCount) {
...
int callService(lua_State* state, const std::string& name,
                int argumentCount) {
    if (state == nullptr || argumentCount < 0 ||
        argumentCount > lua_gettop(state)) {
    ...
    const int stackBase = lua_gettop(state) - argumentCount;
    sol::state_view lua(state);
    const sol::object rawService = runtimeServiceRegistry(lua)
                                       .raw_get<sol::object>(name);
