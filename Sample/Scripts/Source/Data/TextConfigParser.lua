...
local function textFontFromData(value, sourceName)
    ...
    if not bool(path) then
        ...
        return assert(engineDefaultFont, "Default font is unavailable for text config " .. sourceName)
    end
    return assert(ManagerFunctions.loadFont(path), "Font not found for text config " .. sourceName .. ": " .. path)
end
...
local function textGradientFromData(data, value, sourceName)
    ...
    return TextGradientConfig.new({ enabled = enabled, direction = direction, curve = curve })
end
...
function TextConfigParser.BuildPlain(data, name, value)
...
function TextConfigParser.BuildRich(data, name, value)
    local sourceName = tostring(name)
