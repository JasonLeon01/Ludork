...
sol::object compileGraphTemplate(sol::state_view lua, const sol::table& data,
    ...
    return runtimeIdentityValue(lua, runtimeProviders().compileBlueprintGraph(
                                         retainRuntimeIdentity(graphData),
                                         retainRuntimeIdentity(classType)));
}
...
sol::object instantiateClassGraph(sol::state_view lua,
    ...
    return runtimeIdentityValue(lua,
                                runtimeProviders().instantiateBlueprintGraph(
                                    retainRuntimeIdentity(graphTemplate),
                                    retainRuntimeIdentity(parent)));
}
...
sol::object resolveConfigValue(sol::state_view lua, const sol::object& value,
    ...
    return sol::make_object(
        lua, runtimeProviders().config(rawConfig.as<std::string>(),
                                       rawSetting.as<std::string>()));
}
...
void initializeGeneratedInstance(lua_State* state, const std::string& classPath,
    ...
    if (rawInit.is<sol::protected_function>()) {
        ...
        static_cast<void>(ludork::runtime::detail::invokeRuntimeFunction(
            state, rawInit, values, "runtime class init arguments"));
        lua_settop(state, stackBase);
    }
    ludork::engine::components::normaliseInstanceComponents(
        ludork::runtime::binding::readLuaValue<RuntimeValue>(self));
}
