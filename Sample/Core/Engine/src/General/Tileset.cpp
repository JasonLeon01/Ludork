...
template <typename T>
bool assignIfPresent(const TilesetData& data, const std::string& key,
                     T& target) {
...
std::vector<MaterialData> materialDataVectorFromMaterials(
...
std::vector<Material> materialsFromMaterialDataVector(
    const std::vector<MaterialData>& items) {
...
Tileset::Tileset(std::string name, std::string fileName,
                 std::vector<bool> passable, std::vector<Material> materials,
...
TilesetData Tileset::asDict() const {
    return {
        ...
        {"fileName", fileName},
        {"passable", passable},
        {"materials", materialDataVectorFromMaterials(materials)},
        {"dir4", dir4},
    };
}
...
Tileset Tileset::fromData(TilesetData data) {
    ...
    if (it != data.end()) {
        if (const auto materialData =
                std::get_if<std::vector<MaterialData>>(&it->second)) {
