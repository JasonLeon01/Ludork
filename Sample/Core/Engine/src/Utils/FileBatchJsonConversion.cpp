...
class RuntimeValueJsonDocument final
    : public ludork::standard::FileBatchJsonDocument {
public:
    explicit RuntimeValueJsonDocument(RuntimeValue value)
        : value_(std::make_shared<const RuntimeValue>(std::move(value))) {}
...
private:
    std::shared_ptr<const RuntimeValue> value_;
};

ludork::standard::FileBatchParsedJson parseFileBatchJsonDocument(
    const std::string& content) {
    return std::make_shared<RuntimeValueJsonDocument>(parseJSONText(content));
}
