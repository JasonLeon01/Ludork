...
sf::Vector2i pixelPosition(const sf::WindowBase& window, NSView* view,
    ...
    const NSRect bounds = [view bounds];
    const NSPoint viewPosition =
        [view convertPoint:windowPosition fromView:nil];
    const sf::Vector2f scale = coordinateScale(window, view);
    const CGFloat top = [view isFlipped]
                            ? viewPosition.y - NSMinY(bounds)
                            : NSMaxY(bounds) - viewPosition.y;
    return {static_cast<int>(std::lround(
                (viewPosition.x - NSMinX(bounds)) * scale.x)),
...
void enqueueScrollEvent(NSEvent* event) {
    ...
    const CGFloat deltaX =
        precise ? [event scrollingDeltaX] : [event deltaX];
    const CGFloat deltaY =
        precise ? [event scrollingDeltaY] : [event deltaY];
...
bool setMousePosition(sf::WindowBase& window,
    ...
    @autoreleasepool {
        ...
        const CGFloat y = [view isFlipped] ? NSMinY(bounds) + top
                                           : NSMaxY(bounds) - top;
        const NSPoint windowPoint =
            [view convertPoint:NSMakePoint(x, y) toView:nil];
...
std::vector<ScrollEvent> consumeScrollEvents(sf::WindowBase& window) {
    @autoreleasepool {
        ...
        for (const PendingScrollEvent& event : pending) {
            ...
            events.push_back(
                {delta, pixelPosition(window, view, event.location),
                 event.precise});
        }
        return events;
    }
