...
RuntimeValue runtimeNumber(const NumericValue& value) {
    if (value.integer) {
        ...
        return RuntimeValue(static_cast<std::int64_t>(value.value));
    }
...
NumericValue numericValue(const RuntimeValue& value,
...
NumericValue resolveMagnitude(const GameplayModifier& modifier,
    ...
    if (magnitude.getIf<RuntimeIdentityPtr>() != nullptr) {
        const std::vector<RuntimeValue> results = invokeCallable(
            magnitude, {runtimeObject(spec),
...
struct AbilitySystemComponent::State {
    ...
    std::unordered_map<std::string, RuntimeIdentityPtr> constraints;
    ...
    State(AbilitySystemComponent* ownerComponent, RuntimeValue systemOwner,
        ...
        for (const std::string& name : attributeSet->getAttributeNames()) {
            ...
            baseValues.emplace(name, std::move(value));
        }
    }
    ...
    NumericValue validateNumeric(const std::string& name,
    ...
    NumericValue resolveAttribute(
        ...
        if (constraint != constraints.end()) {
            const std::vector<RuntimeValue> results =
                invokeCallable(RuntimeValue(constraint->second),
                               {runtimeNumber(current), selfValue(),
                                RuntimeValue(resolvedValues)});
            if (results.size() != 1) {
                throw std::invalid_argument(
    ...
    RuntimeValue::Map preview(
        const RuntimeValue::Map& bases,
        const std::shared_ptr<GameplayEffectSpec>& pendingSpec = {},
    ...
    void notify(const std::string& name, const RuntimeValue& oldValue,
        ...
        for (const Listener& listener : entries->second) {
            ...
            arguments.insert(arguments.end(), listener.params.begin(),
                             listener.params.end());
            static_cast<void>(invokeCallable(RuntimeValue(listener.callback),
                                             std::move(arguments)));
        }
    }

    bool applyCurrentValues(
        const RuntimeValue::Map& values, const std::string& source,
        const RuntimeValue::Map* oldBases = nullptr,
        const RuntimeValue::Map* newBases = nullptr,
        ...
        try {
            for (const std::string& name : numericAttributes) {
                ...
                attributeSet->setAttributeValue(name, value->second);
            }
        } catch (...) {
        ...
        for (const std::string& name : numericAttributes) {
            ...
            if (source == "Base") {
                const RuntimeValue oldBase = oldBases->at(name);
                const RuntimeValue newBase = newBases->at(name);
    ...
    void commitBases(const RuntimeValue::Map& bases,
                     const RuntimeValue::Map* oldValueOverrides = nullptr) {
        const RuntimeValue::Map oldBases = baseValues;
        const RuntimeValue::Map currentValues = preview(bases);
    ...
    std::pair<RuntimeValue::Map, bool> instantBases(
        const std::shared_ptr<GameplayEffectSpec>& spec) const {
        RuntimeValue::Map result = baseValues;
        bool changed = false;
        for (const GameplayModifier& modifier : spec->effect->modifiers) {
            ...
            const RuntimeValue value = runtimeNumber(base);
...
RuntimeValue AbilitySystemComponent::getNumericAttributeBase(
    const std::string& name) const {
    state_->requireNumericAttribute(name);
    return state_->baseValues.at(name);
}
...
void AbilitySystemComponent::setNumericAttributeBase(
    ...
    RuntimeValue::Map bases = state_->baseValues;
    bases[name] = value;
...
void AbilitySystemComponent::addAttributeChangeListener(
    ...
    state_->listeners[name].push_back({std::move(callback), std::move(params)});
}
...
void AbilitySystemComponent::setNumericAttributeConstraint(
    ...
    if (callback == nullptr) {
        ...
        state_->constraints[name] = std::move(callback);
    }
    const RuntimeValue::Map current = state_->preview(state_->baseValues);
...
std::optional<int> AbilitySystemComponent::applyGameplayEffectSpec(
    ...
    if (existing != nullptr) {
        if (spec->effect->stackingPolicy == "Aggregate") {
            ...
            const RuntimeValue::Map current = state_->preview(
    ...
    const RuntimeValue::Map current = state_->preview(state_->baseValues, spec);
...
bool AbilitySystemComponent::removeActiveGameplayEffect(
    ...
    const RuntimeValue::Map current =
