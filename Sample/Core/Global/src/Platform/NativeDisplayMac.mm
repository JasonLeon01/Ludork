...
std::optional<sf::Vector2u> maximumClientSizeForScreen(
    ...
    const NSRect contentRect = [NSWindow
        contentRectForFrameRect:NSMakeRect(0.0, 0.0,
...
void setWindowedFramePlacement(
    ...
    @autoreleasepool {
        ...
        const NSRect visibleFrame = [screen visibleFrame];
        const NSRect frame = [window frame];
        const CGFloat x = frame.size.width <= visibleFrame.size.width
                              ? std::clamp(
                                    static_cast<CGFloat>(placement.topLeft.x),
                                    NSMinX(visibleFrame),
                                    NSMaxX(visibleFrame) - frame.size.width)
                              : NSMinX(visibleFrame);
        const CGFloat top = frame.size.height <= visibleFrame.size.height
                                ? std::clamp(
                                      static_cast<CGFloat>(placement.topLeft.y),
