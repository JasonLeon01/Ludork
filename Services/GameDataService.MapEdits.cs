...
public sealed partial class GameDataService
{
    ...
    public int? AddMapActor(string mapKey, string layerName, JsonObject source, int x, int y, JsonObject? overrides = null)
    {
        ...
        if (actors?.OfType<JsonObject>().Any(actor => actor["position"] is JsonArray position
            ...
            return null;
        string reference = getString(source["bp"]) ?? string.Empty;
        if (reference.Length == 0)
            return null;
        JsonObject candidate = (JsonObject)source.DeepClone();
        string tag = MapTagService.CreateDefault(this, mapKey, reference, x, y);
        candidate["tag"] = tag;
    ...
    public string? RenameMapActorTag(string mapKey, string layerName, int index, string expectedTag, string requestedTag)
    {
        if (!tryGetEditableActor(mapKey, layerName, index, expectedTag, out JsonObject map, out _))
            return null;
        string tag = MapTagService.MakeUnique(this, mapKey, requestedTag, layerName, index);
    ...
    private bool commitMapEdits(string mapKey, JsonObject map, IReadOnlyList<JsonDataEdit> edits)
    {
    ...
    private bool tryGetEditableActor(string mapKey, string layerName, int index, string expectedTag,
        out JsonObject map, out JsonObject actor)
    {
        ...
        if (map?["layers"]?[layerName] is not JsonObject layer
            ...
            || !string.Equals(getString(current["tag"]) ?? string.Empty, expectedTag, StringComparison.Ordinal))
            return false;
        actor = current;
        return true;
    }

    private static void addRemoveActorOverrides(JsonObject map, string tag, string? name, ICollection<JsonDataEdit> edits)
    {
    ...
    private static JsonNode? readMapCell(JsonNode? grid, int x, int y)
    {
        return grid is JsonArray rows && y < rows.Count && rows[y] is JsonArray row && x < row.Count ? row[x] : null;
    }

    private static void addMapTileEdit(JsonObject layer, string layerName, string gridName, MapTileEdit cell,
    ...
    private static bool isMapPositionValid(JsonObject map, int x, int y)
    {
