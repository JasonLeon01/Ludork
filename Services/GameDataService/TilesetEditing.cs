...
public sealed partial class GameDataService
{
    ...
    {
        return RenameDocumentResource("AutoTiles", oldKey, newKey);
    }
    ...
    public bool PasteTileset(string key, bool isAutoTile, JsonObject source)
    {
        ...
        if (!canCreateDocument(isAutoTile ? "AutoTiles" : "Tilesets", key))
            return false;
        JsonObject copy = (JsonObject)source.DeepClone();
        copy["name"] = key;
        RecordDocumentSnapshot(isAutoTile ? "AutoTiles" : "Tilesets", key);
        target.Add(key, copy);
        completeTilesetEdit();
        return true;
    }
    ...
    {
        return DeleteDocumentResource(isAutoTile ? "AutoTiles" : "Tilesets", key);
    }
