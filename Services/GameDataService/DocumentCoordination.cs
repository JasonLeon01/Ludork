...
{
    private readonly HashSet<string> deletedDocumentPaths = new(OperatingSystem.IsWindows()
        ? StringComparer.OrdinalIgnoreCase : StringComparer.Ordinal);

    private HistoryResult restoreDocumentHistory(EditorDocument document, DocumentHistoryEntry entry, bool undo)
    {
    ...
    private bool commitResourceChange(string section, string oldKey, string newKey, JsonObject candidate,
        JsonObject? worldCandidate = null, bool recordSource = true)
    {
        ...
        if (renamed)
        {
            ...
            if (section == "WorldMaps")
            {
                ...
                foreach (string childKey in getWorldChildren(oldKey).ToArray())
                {
        ...
        if (section == "Maps" && TryGetWorldForMap(oldKey, out string worldKey))
        {
        ...
        foreach (KeyValuePair<EditorDocument, (string Key, JsonObject Data)> change in changes)
        {
            ...
            {
                sections[document.Section].Data.Remove(previousKey);
                sections[document.Section].Data[change.Value.Key] = change.Value.Data;
    ...
    private JsonObject prepareWorldForMapChange(string worldKey, string oldKey, string newKey, JsonObject candidate)
    {
        JsonObject world = getWorldMap(worldKey)!;
        WorldMapValidationResult current = ValidateWorldMap(worldKey);
        ...
        children[newKey] = createMapCatalogEntry(newKey, MapCatalogEntryKind.WorldChildMap, worldKey, candidate);
        IReadOnlyList<string> order = worldMapValidation.TryMergeLayerOrder(worldKey, placements, children)
            ?? throw new InvalidDataException("The map layers conflict with the world composition.");
        JsonObject result = replaceWorldComposition(world, order, placements);
        WorldMapValidationResult validation = worldMapValidation.Validate(worldKey, result, children);
        if (!validation.IsValid)
            throw new InvalidDataException(formatWorldMapValidation(validation));
        return result;
    }

    private void updateDocumentCatalog(string section, string oldKey, string newKey, JsonObject? data)
    {
        if (data is null)
        {
            if (section == "Maps")
            {
                removeMapCatalogEntry(oldKey.Contains('/') ? MapCatalogEntryKind.WorldChildMap : MapCatalogEntryKind.StandaloneMap, oldKey);
                removeLoadedMapMetadata(oldKey);
            }
            else if (section == "WorldMaps")
            {
                removeMapCatalogEntry(MapCatalogEntryKind.WorldMap, oldKey);
        ...
        if (section == "Maps")
        {
            ...
            setMapCatalogEntry(createMapCatalogEntry(newKey, worldKey is null
                ? MapCatalogEntryKind.StandaloneMap : MapCatalogEntryKind.WorldChildMap, worldKey, data));
            if (oldKey != newKey)
                rekeyLoadedMapMetadata(oldKey, newKey);
            updateLoadedMapMetadata(newKey, data);
        }
        else if (section == "WorldMaps")
        {
            removeMapCatalogEntry(MapCatalogEntryKind.WorldMap, oldKey);
            setMapCatalogEntry(new MapCatalogEntry(newKey, getString(data["worldName"]) ?? newKey,
            ...
            if (document is not null && document.SavedState.InternalData is not null && document.SavedPath != document.Path)
                pendingWorldDirectoryMoves[newKey] = Path.GetDirectoryName(document.SavedPath)!;
        }
    }

    private static void applyDocumentDifference(JsonObject target, JsonObject before, JsonObject after)
    {
    ...
    private void validateRestoredReferences(EditorDocument document, JsonObject candidate)
    {
        JsonObject original = sections[document.Section].Data[document.Key];
        LuaMetadataService metadata = new(ProjectPath);
        using BlueprintClassResolver resolver = new(this, metadata);
        using ReferenceIndexService references = new(this, metadata, resolver);
        try
        {
            sections[document.Section].Data[document.Key] = candidate;
        ...
        {
            sections[document.Section].Data[document.Key] = original;
        }
    }
}
