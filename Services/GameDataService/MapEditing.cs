...
public sealed partial class GameDataService
{
    ...
    public bool CreateMap(MapInfo info)
    {
        ...
        if (!isValidMapChildName(key) || !canCreateDocument("Maps", key) || sections["WorldMaps"].Data.ContainsKey(key)
        ...
        if (Directory.Exists(Path.ChangeExtension(mapPath, null)))
            return false;
        RecordDocumentSnapshot("Maps", key);
    ...
    public bool UpdateMap(string currentKey, MapInfo info, Func<IReadOnlyList<ReferenceRewrite>>? prepareReferences = null)
    {
        ...
        return commitResourceChange("Maps", currentKey, newKey, candidate, worldCandidate);
    }
    ...
    public string? PasteMap(JsonObject source, string sourceKey)
    {
        ...
        for (int index = 1; !canCreateDocument("Maps", copyKey); index += 1)
        ...
        RecordDocumentSnapshot("Maps", copyKey);
    ...
    {
        return DeleteDocumentResource("Maps", normaliseMapKey(key));
    }
    ...
    public bool CreateAnimation(string key, string name)
    {
        ...
        if (string.IsNullOrWhiteSpace(key) || key.EndsWith(".anim", StringComparison.OrdinalIgnoreCase)
            || !canCreateDocument("Animations", key))
            return false;
        RecordDocumentSnapshot("Animations", key);
    ...
    public bool CreateCurve(string key, string name, string type = "curve")
    {
        ...
        if (string.IsNullOrWhiteSpace(key)
            || !isCurveType(type)
            || !canCreateDocument("Curves", key))
        {
            return false;
        }
        int componentCount = curveComponentCount(type);
        RecordDocumentSnapshot("Curves", key);
    ...
    public bool CreateTextConfig(string key, string type, string name)
    {
        ...
        if (string.IsNullOrWhiteSpace(key)
            ...
            || !canCreateDocument("TextConfigs", key))
        {
            return false;
        }
        RecordDocumentSnapshot("TextConfigs", key);
    ...
    {
        bool changed = false;
        foreach (string key in keys.Select(normalizeJsonKey).Distinct(StringComparer.Ordinal).ToArray())
            changed |= DeleteDocumentResource("TextConfigs", key);
        return changed;
    }
    ...
    {
        return RenameDocumentResource("Tilesets", oldKey, newKey);
    }

    public bool CreateTileset(string key)
    {
        key = normalizeDataKey(key);
        if (!canCreateDocument("Tilesets", key))
            return false;
        RecordDocumentSnapshot("Tilesets", key);
    ...
    {
        key = normalizeDataKey(key);
        if (!canCreateDocument("AutoTiles", key))
            return false;
        RecordDocumentSnapshot("AutoTiles", key);
