...
[Export Package](../../../.github/workflows/export-package.yml) packages at 02:00 and 14:00 UTC+8 and on manual dispatch, retaining artifact names and seven-day retention. Scheduled and manual temporary packages share serialization; only scheduled runs skip an unchanged SHA against the latest successful dual-platform default-branch temporary package. Successful manual default-branch runs update that baseline; other refs, tag packages and unsuccessful or skipped runs do not. Export Editor and Export Package build on clean runners without restoring or saving dependency or build caches. Preserve only the artifact handoffs within the current workflow run; do not enable reuse of previous managed, launcher, tool or native outputs. Scheduled and manual runs package with `--dev`; `v*` tag pushes package with `--release`, build Windows MSI and macOS DMG assets and create or update a draft Release with generated notes; never publish it automatically or overwrite a published release. Only the upload job needs `contents: write`. See [tools/README.md](../../../tools/README.md#pull-requests-and-automated-packages) for the operational contract.

The three `Export Editor` entries omit templates and their native builds while retaining editor tools and macOS signing. The three `Export Package` entries include templates. Both combined entries run at the same 02:00 and 14:00 UTC+8 schedule, with separate workflow/success-marker baselines and concurrency groups per mode and platform. Manual runs always build. Only `Export Package` handles tag pushes and creates draft releases with complete packages. Keep artifact names distinct and require consumers to find the latest available successful target-platform artifact.
