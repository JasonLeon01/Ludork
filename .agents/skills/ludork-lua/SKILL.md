---
name: ludork-lua
description: Edit or review Ludork Lua runtime, Standard globals, LuaLS declarations, Script Mixins and Default Gameplay. Use for Lua semantics and gameplay contracts, including their native implementations.
---
...
Before adding a generic Lua helper, search the relevant declarations under `Game/Scripts/stub`: `Standard.d.lua`, `Engine.d.lua`, `GlobalCore.d.lua`, `GlobalFunctions.d.lua` and `LuaSF.d.lua`. Read matching signatures and implementations as needed; unrelated APIs need not be loaded. Reuse an API only when inputs, results, mutation, ordering, nil, equality and error semantics match. Domain-specific local functions remain appropriate. Add a missing reusable primitive once in the owning Native layer, with declarations, locale docs and focused verification.

For known bound value types that expose instance copying (such as LuaSF vectors, rectangles, colours and transforms), call `value:copy()` or `value:deepcopy()` directly. Use global `copy`/`deepcopy` for Lua tables and dynamic value graphs; their native protocol delegates bound values to the same LuaGlue copy entry points and preserves container aliases and cycles. Preserve documented identity/ownership policies for resource handles. Use `Class.isInstance` and `Class.isSubclass` directly, `bool` for ordinary truth tests, and explicit nil comparisons for absence/presence or protocol sentinels. Do not alias these checks, introduce `isinstance`, compare Lua `type` results or use `next` as an emptiness test. Use `math.type` only after a number check. Native `sf.*` values do not accept tuple/list/Pair adapters; `TypeAdapter` is unsupported.
...
- Complete direct `require` calls in established order, then add a blank line before member aliases. Load root native modules in `Engine`, `GlobalCore`, `GlobalFunctions` order; use full paths for other modules. Lua Blueprint function libraries under `Scripts/GlobalFunctions` defer every `require`, including native modules, until a function needs it; no `require` executes at module scope. Do not add directory `init.lua` aggregators, chain `require(...).Member`, or move cycle-breaking delayed loads out of their functions. Use injected `sf` directly.
