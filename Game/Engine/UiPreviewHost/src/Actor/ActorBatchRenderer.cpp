...
actor_batch_detail::FileStamp shaderStamp(const std::string& path) {
    const actor_batch_detail::FileStamp requested = fileStamp(path);
    if (requested.exists) {
        return requested;
    }
...
void ActorBatchRenderer::Impl::reset(const std::filesystem::path& projectPath) {
...
RuntimeData ActorBatchRenderer::Impl::render(const RuntimeData::Map& request,
    ...
    const RuntimeData::Array& itemValues =
        ludork::runtime::value_reader::requireArray(
            ludork::runtime::value_reader::requireValue(request, "items",
                                                        "Actor render request"),
    ...
    for (std::size_t pageIndex = 0; pageIndex < layouts.size(); ++pageIndex) {
    ...
    for (std::size_t pageIndex = 0; pageIndex < layouts.size(); ++pageIndex) {
        ...
        sharedPixels.insert(sharedPixels.end(), pixels.begin(), pixels.end());
        pages.emplace_back(object({
            {"width", RuntimeData(static_cast<std::int64_t>(layout.size.x))},
            {"height", RuntimeData(static_cast<std::int64_t>(layout.size.y))},
            {"stride",
    ...
    for (RuntimeData& pageValue : pages) {
        RuntimeData::Map& page = *pageValue.getMutableIf<RuntimeData::Map>();
    ...
    for (const PackedActorVisual& visual : visuals) {
        items.emplace_back(object({
            ...
            {"x", RuntimeData(static_cast<std::int64_t>(visual.position.x))},
            {"y", RuntimeData(static_cast<std::int64_t>(visual.position.y))},
...
ActorBatchRenderer::Impl::PackedActorVisual
ActorBatchRenderer::Impl::parseVisual(const RuntimeData& value,
    ...
    actor_batch_detail::ActorVisualRequest visual{
        ...
        sf::IntRect({ludork::runtime::value_reader::requireInt(
                    ...
                    {width, height}),
        shaderText.empty() ? std::string{} : actorShaderPath(shaderText), hue};
    return {std::move(visual)};
}

std::vector<ActorBatchRenderer::Impl::AtlasLayout>
ActorBatchRenderer::Impl::pack(std::vector<PackedActorVisual>& visuals,
...
std::vector<std::uint8_t> ActorBatchRenderer::Impl::renderPage(
...
sf::RenderTexture& ActorBatchRenderer::Impl::pageBuffer(
    const sf::Vector2u& size, std::size_t pageIndex) {
...
void ActorBatchRenderer::Impl::renderVisual(sf::RenderTexture& target,
                                            PackedActorVisual& packed,
                                            float time) {
...
const ActorBatchRenderer::Impl::TextureCacheEntry&
ActorBatchRenderer::Impl::loadTexture(const std::string& path) {
    const std::string& key = path;
    const actor_batch_detail::FileStamp stamp = fileStamp(key);
...
const ActorBatchRenderer::Impl::ShaderCacheEntry&
ActorBatchRenderer::Impl::loadShader(const std::string& path) {
    const std::string& key = path;
    const actor_batch_detail::FileStamp stamp = shaderStamp(path);
...
sf::RenderTexture& ActorBatchRenderer::Impl::effectBuffer(
...
std::uint64_t ActorBatchRenderer::Impl::effectBufferKey(
    const sf::Vector2i& size) {
...
void ActorBatchRenderer::Impl::pruneEffectBuffers(
...
sf::Shader& ActorBatchRenderer::Impl::requireErrorShader() {
...
void main()
{
    ...
    gl_FragColor = vec4(source.r, 0.0, source.b, source.a) * sf_FrontColor;
        ...
        if (!errorShader_->loadFromMemory(source, sf::Shader::Type::Fragment)) {
            errorShader_.reset();
            throw std::runtime_error(
...
void ActorBatchRenderer::Impl::drawError(sf::RenderTexture& target,
...
void ActorBatchRenderer::Impl::bindActorShader(sf::Shader& shader,
                                               const sf::Texture& texture,
                                               const sf::IntRect& rect,
                                               float time) {
...
void ActorBatchRenderer::Impl::bindHueShader(sf::Shader& shader, float hue) {
