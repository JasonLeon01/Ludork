...
struct RuntimeNode {
    ...
    std::shared_ptr<AssetImpl> nestedImpl;
...
struct AssetImpl {
    ...
    std::unordered_map<std::string, std::shared_ptr<AssetImpl>> nestedImpls;
    ...
    std::unordered_map<std::string, std::shared_ptr<ActiveAnimation>>
        activeAnimations;
    std::weak_ptr<AssetImpl> parentImpl;
...
using UiCanvasSlotData =
    ludork::engine::ui_asset_runtime_impl::RuntimeNode::CanvasSlotData;
