...
from .pysf import Sound, Sprite, Texture, RenderTexture, Vector2f, Vector2u, Color, Image, seconds
    ...
    def _loadTexture(self, path: str) -> Texture:
        try:
            ...
            if "/" in path:
                ...
                tex = Texture(os.path.join(sub, filename))
                tex.setSmooth(True)
                return tex
            tex = Texture()
            if not tex.loadFromFile(path):
...
def exportSEPartToDict(part: SEPart) -> Dict[str, Any]:
    return {
        "path": part.path,
        "beginTime": part.beginTime,
        "endTime": part.endTime,
        "status": exportSEStatusToDict(part.status),
    }


def exportEffectToDict(effect: Effect) -> Dict[str, Any]:
    return {
        "frameRate": effect.frameRate,
        "startTime": effect.startTime,
        "endTime": effect.endTime,
        "frames": effect.frames,
        "frameTimes": effect.frameTimes,
        "audio": [exportSEPartToDict(part) for part in effect.audio],
    }
