...
local PlayerClass
local function loadPlayerClass()
    if PlayerClass == nil then
        PlayerClass = require("Source.MapActors.Player")
    end
    return PlayerClass
end
...
function Player.GetPlayer()
    return loadContext().RequireGameInstance():getPlayer()
end
...
function Player.GetPlayerFrontPosition()
    ...
    local x = position.x
    local y = position.y
    if direction == loadEngine().Direction.UP then
        y = y - 1
    elseif direction == loadEngine().Direction.LEFT then
        x = x - 1
    elseif direction == loadEngine().Direction.RIGHT then
        x = x + 1
    else
        y = y + 1
    end
...
function Player.GetPlayerAttrRef(attrName)
    ...
    local NodeUtils = require("GlobalFunctions.Utils")
...
function Player.HealPlayer(amount)
    ...
    local player = Player.GetPlayer()
    if player ~= nil then
        loadEffects().ApplyInstantModifier(player, "Blueprint.Heal", "HP", "Add", amount)
    end
end
...
function Player.DamagePlayer(amount)
    ...
    local player = Player.GetPlayer()
    if player ~= nil then
        loadEffects().ApplyInstantModifier(player, "Blueprint.Damage", "HP", "Add", -amount)
    end
end
...
function Player.AddHP(amount)
    ...
    local player = Player.GetPlayer()
    if player ~= nil then
        loadEffects().ApplyInstantModifier(player, "Blueprint.AddHP", "HP", "Add", amount)
    end
end
...
function Player.AddGold(amount)
    ...
    local player = Player.GetPlayer()
    if player ~= nil then
        loadEffects().ApplyInstantModifier(player, "Blueprint.AddGold", "GOLD", "Add", amount)
    end
end
...
function Player.AddATK(amount)
    ...
    local player = Player.GetPlayer()
    if player ~= nil then
        loadEffects().ApplyInstantModifier(player, "Blueprint.AddATK", "ATK", "Add", amount)
    end
end
...
function Player.AddDEF(amount)
    ...
    local player = Player.GetPlayer()
    if player ~= nil then
        loadEffects().ApplyInstantModifier(player, "Blueprint.AddDEF", "DEF", "Add", amount)
    end
end
...
function Player.AddEXP(amount)
    ...
    local player = Player.GetPlayer()
    if player ~= nil then
        loadEffects().ApplyInstantModifier(player, "Blueprint.AddEXP", "EXP", "Add", amount)
    end
end

function Player.MeetPlayer(actors)
    return loadPlayerClass().MeetPlayer(actors, Player.GetPlayer())
end
