...
{
    internal void loadMapCatalogCache()
    {
        loadedMapCatalogCache.Clear();
        nextMapCatalogCache.Clear();
        if (!store.CacheMapCatalog)
            return;
    ...
    internal void saveMapCatalogCache()
    {
        if (!store.CacheMapCatalog)
            return;
        string path = getMapCatalogCachePath();
        try
        {
            ...
            File.WriteAllText(path, root.ToJsonString(ProjectDataStore.WriteOptions) + Environment.NewLine);
        }
        catch (Exception exception) when (
    ...
    internal string getMapCatalogCachePath()
    {
        return Path.Combine(store.ProjectPath, ProjectToolConstants.EditorCacheDirectory, "EditorMapCatalog.json");
    }

    internal JsonObject? readMapFile(string path, bool requireType)
    {
        ...
        try
        {
            if (JsonNode.Parse(File.ReadAllText(path)) is not JsonObject data)
                return null;
            string? type = ProjectDataStore.getString(data["type"]);
    ...
    internal static bool isValidCatalogGrid(IReadOnlyList<int>? rows, int width, int height)
    {
    ...
    {
        string mapsRoot = Path.GetFullPath(Path.Combine(store.ProjectPath, "Data", "Maps"));
    ...
    internal void setMapCatalogEntry(MapCatalogEntry entry)
    {
        catalogDocuments[getMapCatalogDataKey(entry.Kind, entry.Key)] =
            createMapCatalogData(entry);
    }

    internal static JsonObject createMapCatalogData(MapCatalogEntry entry)
    {
    ...
    internal void removeMapCatalogEntry(MapCatalogEntryKind kind, string key)
    {
        catalogDocuments.Remove(getMapCatalogDataKey(kind, key));
    }

    internal static string getMapCatalogDataKey(MapCatalogEntryKind kind, string key)
    {
        return kind + ":" + key;
    }

    internal static MapCatalogEntry? readMapCatalogEntry(JsonObject data)
    {
        string? key = ProjectDataStore.getString(data["key"]);
        string? displayName = ProjectDataStore.getString(data["displayName"]);
        string? kindName = ProjectDataStore.getString(data["kind"]);
        ...
        return new MapCatalogEntry(
            ...
            displayName,
            kind,
            ProjectDataStore.getString(data["worldKey"]),
    ...
    internal static MapCatalogEntry createMapCatalogEntry(
        string key,
        ...
        JsonObject map)
    {
        ...
        string? mapName = ProjectDataStore.getString(map["mapName"]);
        return new MapCatalogEntry(
            key,
    ...
    internal static IReadOnlyList<string> readActorTags(JsonObject map)
    {
        List<string> result = [];
    ...
    internal static void appendActorTags(JsonArray actors, ICollection<string> result)
    {
    ...
    internal static bool tryReadMapShape(
        JsonObject map,
        out int width,
        out int height,
    ...
    internal static bool isValidMapGrid(JsonNode? value, int width, int height)
    {
    ...
    internal static bool mapMatchesCatalogEntry(JsonObject map, MapCatalogEntry entry)
    {
    ...
    internal static IReadOnlyList<string> readStringArray(JsonNode? value)
    {
        ...
        {
            string? text = ProjectDataStore.getString(item);
            if (text is null)
                return [];
            result.Add(text);
        }
    ...
    internal void setWorldCatalogLayerOrder(string worldKey, IReadOnlyList<string> layerOrder)
    {
        string catalogKey = MapDataService.getMapCatalogDataKey(MapCatalogEntryKind.WorldMap, worldKey);
        if (!catalogDocuments.TryGetValue(catalogKey, out JsonObject? data))
            return;
