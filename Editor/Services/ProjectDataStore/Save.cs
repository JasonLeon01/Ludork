...
public sealed partial class ProjectDataStore
{
    public SaveResult SaveAllModified()
    {
        ...
        WorldMapMutationResult worldValidation = Worlds.ValidateAllWorldMaps();
    ...
    internal IReadOnlyList<string> validateGameAssetPaths()
    {
        ...
        foreach (KeyValuePair<string, JsonObject> entry in sections["AutoTiles"])
            validateAssetPath(entry.Value["fileName"], $"AutoTiles/{entry.Key}.fileName", errors);
        foreach (KeyValuePair<string, JsonObject> entry in sections["Animations"])
        {
        ...
        foreach (KeyValuePair<string, JsonObject> entry in sections["Configs"])
            validateConfigAssetPaths(entry.Key, entry.Value, errors);
        foreach (KeyValuePair<string, JsonObject> entry in sections["Maps"])
            validateMapAssetPaths(entry.Key, entry.Value, errors);
        foreach (KeyValuePair<string, JsonObject> entry in sections["WorldMaps"])
        {
        ...
        foreach (KeyValuePair<string, JsonObject> entry in sections["TextConfigs"])
            validateAssetPath(entry.Value["font"], $"TextConfigs/{entry.Key}.font", errors);
        return errors;
    }

    internal static void validateConfigAssetPaths(
        string key,
        JsonObject config,
    ...
    internal static void validateMapAssetPaths(
        string key,
        JsonObject map,
        ICollection<string> errors)
    {
    ...
    internal static void validateAssetPath(
        JsonNode? value,
        string path,
        ICollection<string> errors)
    {
        ...
        JsonArray autoTiles)
    {
        ...
        EditorDocumentCollection maps = sections["Maps"];
        ...
        if (!result.Success)
            return result;
        Maps.updateLoadedMapMetadata(mapKey, candidate, path);
        originData["Maps"][mapKey] = (JsonObject)candidate.DeepClone();
        MarkDocumentSaved("Maps", mapKey);
        transaction.Commit();
        refreshModifiedState();
        Maps.NotifyMapContentChanged(mapKey);
        DataSaved?.Invoke(this, EventArgs.Empty);
        return result;
    }

    internal bool hasPendingPathDependencies(EditorDocument document)
    {
