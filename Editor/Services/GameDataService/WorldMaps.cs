...
public sealed partial class GameDataService
{
    ...
    public bool CreateWorldMap(string worldKey, WorldMapInfo info)
    {
        ...
        if (info is null
            ...
            || !canCreateDocument("WorldMaps", worldKey)
        ...
        if (!validation.IsValid)
            return false;
        RecordDocumentSnapshot("WorldMaps", worldKey);
    ...
    {
        return isValidMapChildName(newKey) && RenameDocumentResource("WorldMaps", normalizeWorldKey(currentKey), newKey);
    }

    public bool DeleteWorldMap(string worldKey)
    {
        return DeleteDocumentResource("WorldMaps", normalizeWorldKey(worldKey));
    }
    ...
    public bool CreateWorldChildMap(string worldKey, MapInfo info)
    {
        ...
        if (!isValidMapChildName(childName)
            || !canCreateDocument("Maps", worldKey + "/" + childName)
            || !isValidMapSize(info.Width, info.Height)
    ...
    public string? CopyWorldChildMap(string childMapKey)
    {
        ...
        if (!canCreateDocument("Maps", newKey))
            return null;
        ...
        if (hasDuplicateWorldActorTags(worldKey, newKey, copy))
            return null;
        RecordDocumentSnapshot("Maps", newKey);
