...
- File organisation: keep each independent business class, component class and bound type in its own file or Lua module. Use class-named C#/C++ files and preserve existing partial file layouts; Lua window Controllers use `Controller.lua` under the window's same-named module directory. Split C++ declarations and out-of-line member definitions together. Pure-data record/struct collections, interface collections, host-private helpers and genuine utils may stay together; independent `BIND_CLASS` structs follow the bound-type rule; owner-specific nested types stay with their owner. Judge utils by responsibility, not directory name.
