...
- Core bindgen must not generate bespoke binding bodies keyed by business module, type, or member name. Extra conversion capability must be implemented as generic recursive adapters, type traits, or declarative `BIND_*` annotations in `LudorkCoreBinding`; cross-module dependencies are satisfied by including the full type declarations in the binding headers.
...
- Meta-editable fields, `_componentTypes`, spawn `default*` fields, Info `ID` / `_infoType`, and dataclass component fields (such as `lightColour`, `className`) all belong on the class table.
- Correct example: `Character.lua` (`direction` and similar on the class table; `init` only sets `_rectSize` / `_sx` / `_sy`). Incorrect example: assigning `material`, public component fields, or `InfoBase.ID` only inside `init`.
...
- Class and inheritance checks use `Class.isInstance` / `Class.isSubclass` directly; do not create local aliases for them, and do not add a global `isinstance`. Do not use Lua `type` to recognise script classes or native binding classes.
- Lua `type` is reserved for real serialisation/deserialisation boundaries, explicitly declared primitive-type union dispatch, low-level userdata/reflection, and callable classification in `NodeCompiler`. Stable interfaces do not defensively probe for function existence; wrong types should fail on the real call.
...
- When a Lua table is used as a dictionary, each key/value pair occupies its own line and is indented by nesting level; do not compress dictionaries onto one line. Pure array tables may stay on one line when readable.
