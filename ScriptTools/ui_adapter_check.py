...
def _parse_bool(value: str, source: str) -> bool:
    ...
    if token == "false":
        return False
    raise UiAdapterConsistencyError(
        f"{source} must be true or false"
    )
    ...
    for match in pattern.finditer(text):
        ...
        for index in range(opening, len(text)):
            ...
            if char in "\"'":
                ...
                if depth == 0:
                    result.append(
                        (text[opening + 1 : index], match.start())
                    )
                    break
        else:
            raise UiAdapterConsistencyError(
                f"unclosed {name} invocation"
            )
    return result
...
def _cpp_property(value: str, source: str) -> dict[str, object]:
    ...
    if match is None:
        raise UiAdapterConsistencyError(
            f"{source} has an invalid property declaration"
        )
    arguments = split_macro_arguments(match.group(2))
    if len(arguments) != 4:
        raise UiAdapterConsistencyError(
            f"{source} property requires four arguments"
        )
...
def _cpp_controls(project_root: pathlib.Path) -> list[dict[str, object]]:
    ...
    for invocation in macro_invocations(text, ("UI_CONTROL",)):
        ...
        arguments = [
            ...
            for argument in split_macro_arguments(
        ...
        controls.append(
            {
                ...
                "properties": [
                    ...
                    *[
                        ...
                        for property_value in arguments[7:]
                    ],
                ],
            }
        )
...
def _csharp_scalar(value: str, source: str) -> object:
    ...
    raise UiAdapterConsistencyError(
        f"{source} has unsupported scalar {token}"
    )
...
def _csharp_default(value: str, source: str) -> object:
    ...
    raise UiAdapterConsistencyError(
        f"{source} has unsupported default {token}"
    )
...
def _csharp_property(value: str, source: str) -> dict[str, object]:
    ...
    return _property(
        ...
        len(arguments) == 6
        and _parse_bool(arguments[5], f"{source} editorOnly"),
    )
...
def _csharp_controls(
    ...
    controls_start = text.index(
        "private static IReadOnlyList<UiControlDescriptor> "
        "createSystemDescriptors()"
    )
    ...
    for descriptor_value, _ in _function_invocations(
        ...
        if len(arguments) != 6:
            raise UiAdapterConsistencyError(
                f"{path} descriptor requires six arguments"
            )
        properties = _csharp_properties(
            arguments[5],
    ...
    for control in sorted(
        ...
        line = "|".join(
            (
                ...
                (
                    ""
                    if control["slotType"] is None
                    else str(control["slotType"])
                ),
                "",
            )
        )
...
def main(arguments: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="ScriptTools ui-adapter-check"
    )
    parser.add_argument(
        "repository_root",
        nargs="?",
    ...
    print(
        "UI adapter descriptors are consistent: "
        f"{fingerprint}"
    )
    return 0
