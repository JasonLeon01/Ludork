...
| **C++ Source** | You need to change native engine code, add bindings, debug C++, or target iOS or HarmonyOS. | CMake 3.21 or newer and the platform compiler toolchain. |
...
FFmpeg is disabled by default. On Apple platforms, the prebuilt FFmpeg Standalone template targets macOS; use a C++ Source template for iOS. An iOS application statically links its FFmpeg build and therefore has additional LGPL relinking-material obligations; review the bundled FFmpeg notice before distribution.
...
1. Select **New Project**, choose **Standalone** for a prebuilt desktop runtime or **C++ Source** when you need native source access or an iOS or HarmonyOS package, enter a project name and location, then select **Create**.
...
Project templates carry the Ludork licence, third-party notices and complete local licence texts into created projects and generated game packages. Preserve those materials, add notices for your own dependencies and assets, and review the result before distribution.
