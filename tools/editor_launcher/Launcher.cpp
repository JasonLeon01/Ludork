...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR arguments, int) {
    ...
    if (separator == std::wstring::npos) {
        return reportError(L"Resolve editor directory", ERROR_BAD_PATHNAME);
    }
    const std::wstring editorDirectory = launcherPath.substr(0, separator + 1);
    const std::wstring executable = editorDirectory + L"Binaries\\Ludork.exe";
    ...
    const BOOL started = CreateProcessW(
        ...
        editorDirectory.c_str(), &startup.info.StartupInfo, &processInfo);
    const DWORD startError = started ? ERROR_SUCCESS : GetLastError();
    startup.release();
    if (!started) {
        return reportError(L"Start Binaries\\Ludork.exe", startError);
    }
    ...
    if (waitResult == WAIT_FAILED) {
        ...
        return reportError(L"Wait for Binaries\\Ludork.exe", error);
    }
    ...
    if (!GetExitCodeProcess(processInfo.hProcess, &exitCode)) {
        const DWORD error = GetLastError();
        CloseHandle(processInfo.hProcess);
        return reportError(L"Read Binaries\\Ludork.exe exit code", error);
    }
    CloseHandle(processInfo.hProcess);
