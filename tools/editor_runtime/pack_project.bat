...
if not exist "%SCRIPT_TOOLS%" (
    echo ScriptTools was not found in %TOOLS_DIR%
    exit /b 1
)
...
if not exist "%PROJECT_DIR%\CMakeLists.txt" (
    echo CMakeLists.txt was not found: %PROJECT_DIR%\CMakeLists.txt
    exit /b 1
)
...
if not exist "%LUAC%" (
    echo Host luac was not found in %TOOLS_DIR%
    exit /b 1
)
...
:usage
echo Usage: tools\pack_project.bat [--compile-lua] [--encrypt-shaders] [--encrypt-data] ^<project-folder^> [dist-folder]
exit /b 1
