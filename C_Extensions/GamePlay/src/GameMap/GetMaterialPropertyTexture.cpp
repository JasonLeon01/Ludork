  ...
  try {
    ...
    if (width <= 0 || height <= 0) {
      throw std::runtime_error(
          "Invalid size object for C_FillPassabilityImage");
    }
    ...
    CHECK_NULL(tempPyObjects, colorClass,
               "Failed to get Color class from Engine module");
    tempPyObjects.push_back(colorClass);
    PyObject *vec2uClass = PyObject_GetAttrString(engineModule, "Vector2u");
    CHECK_NULL(tempPyObjects, vec2uClass,
               "Failed to get Vector2u class from Engine module");
