...
std::optional<sf::Texture *> getTexture(std::unordered_map<int, sf::Texture *> *textureCache,
    ...
    if (assetIndex >= 0 && assetIndex < assets.size()) {
        ...
        std::filesystem::path assetPath = std::filesystem::path(assetsRoot) / assetName;
...
std::optional<std::tuple<float, float, float, float, float>> getSegmentTransform(py::dict segment, float frameTime) {
    py::dict startFrame = segment.contains("startFrame") ? segment["startFrame"].cast<py::dict>() : py::dict();
    py::dict endFrame = segment.contains("endFrame") ? segment["endFrame"].cast<py::dict>() : py::dict();
    float startTime = startFrame.contains("time") ? startFrame["time"].cast<float>() : 0.0f;
    float endTime = endFrame.contains("time") ? endFrame["time"].cast<float>() : 0.0f;
    ...
    auto startPos = startFrame.contains("position") ? startFrame["position"].cast<std::pair<float, float>>()
                                                    : std::pair<float, float>(0.0f, 0.0f);
    auto endPos = endFrame.contains("position") ? endFrame["position"].cast<std::pair<float, float>>()
                                                : std::pair<float, float>(0.0f, 0.0f);
    ...
    float startRot = startFrame.contains("rotation") ? startFrame["rotation"].cast<float>() : 0.0f;
    float endRot = endFrame.contains("rotation") ? endFrame["rotation"].cast<float>() : 0.0f;
    float curRot = startRot + (endRot - startRot) * factor;

    auto startScale = startFrame.contains("scale") ? startFrame["scale"].cast<std::pair<float, float>>()
                                                   : std::pair<float, float>(1.0f, 1.0f);
    auto endScale = endFrame.contains("scale") ? endFrame["scale"].cast<std::pair<float, float>>()
                                               : std::pair<float, float>(1.0f, 1.0f);
    float curScaleX = startScale.first + (endScale.first - startScale.first) * factor;
    float curScaleY = startScale.second + (endScale.second - startScale.second) * factor;

    return std::make_tuple(curX, curY, curRot, curScaleX, curScaleY);
}

std::pair<float, float> getRotatedSize(float width, float height, float rotation) {
    ...
    return std::make_pair(width * cosValue + height * sinValue, width * sinValue + height * cosValue);
}
...
std::tuple<float, std::vector<py::bytes>, std::vector<py::dict>> C_CompressAnimation(
    ...
    for (int frameIndex = 0; frameIndex < frameCount; ++frameIndex) {
        ...
        for (auto &timeLine : timeLines) {
            ...
            for (auto &segment : segments) {
                if (!segment.contains("type") || segment["type"].cast<std::string>() != "frame") {
                    continue;
                }
                int assetIndex = segment["asset"].cast<int>();
                auto textureObj = getTexture(&textureCache, assets, assetsRoot, assetIndex);
                ...
                auto [rotatedWidth, rotatedHeight] = getRotatedSize(scaledWidth, scaledHeight, curRot);
    ...
    for (auto &timeLine : timeLines) {
        auto &segments = timeLine.at("timeSegments");
        for (auto &segment : segments) {
            if (!segment.contains("type") || segment["type"].cast<std::string>() != "sound") {
                continue;
            }
            int assetIndex = segment["asset"].cast<int>();
...
void ApplyCompressAnimationBinding(py::module &m) {
    m.def("C_CompressAnimation", &C_CompressAnimation, py::arg("zlibModule"), py::arg("frameCount"),
          py::arg("frameStep"), py::arg("frameRate"), py::arg("timeLines"), py::arg("assets"), py::arg("assetsRoot"),
          py::arg("imageFormat"));
}
